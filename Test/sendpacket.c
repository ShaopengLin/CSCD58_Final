...
int warpHeaderAndSendTcp(uint8_t* tcpbuff, int tcpTotalLen, uint32_t* dest_ip, uint8_t* dest_mac){
    ...
    uint8_t* buffer= malloc(sizeof(struct eth_header)+ sizeof(struct ip_header) + tcpTotalLen);
    memcpy(buffer, eth, sizeof(struct eth_header));
    memcpy(buffer + sizeof(struct eth_header), iph, sizeof(struct ip_header));
    memcpy(buffer + sizeof(struct eth_header) + sizeof(struct ip_header), tcpbuff, tcpTotalLen);
    struct sockaddr_ll socket_address;
    memset(&socket_address, 0, sizeof(struct sockaddr_ll));
    socket_address.sll_family = AF_PACKET;
