...
uint8_t *
tcp_gen_packet (uint8_t *data, uint16_t len, uint32_t src_ip, uint32_t dst_ip,
                uint32_t src_port, uint16_t dst_port, uint32_t seq_num,
                uint32_t ack_num, uint8_t doffset, uint8_t flags,
                uint16_t window)
{
  tcp_hdr_t *header = (tcp_hdr_t *)calloc (1, sizeof (tcp_hdr_t));
  header->src_port = htons (src_port);
  header->des_port = htons (dst_port);
  header->seq_num = htonl (seq_num);
  header->ack_num = htonl (ack_num);

  header->doffset = doffset;
  *(((uint8_t *)header) + FLAG_OFS) = flags;
  ...
  p_hdr.zero = 0;
  p_hdr.pro = 6;
  p_hdr.len = len + sizeof (tcp_hdr_t);
  ...
  header->cksum = cksum (cksum_buf,
                         sizeof (tcp_hdr_t) + sizeof (tcp_pseudo_hdr_t) + len);
  free (cksum_buf);
